...
We have 8 observations, out of 99, that have missing records in patient baseline, but we do have the corresponding treatment result register.
...
We can conclude in the four clusters that, even only with contraceptive (Treatment 0), there is a improvement of hirsutism after 12 month of treatment, although not that significant compared with other with antiandrogen.
...
m4 <- gam(FGm12 ~ Treatment + s(FGm0, by=Treatment) + 
          ...
          data = hirs, method = "REML")
summary(m4) #it seems to be the best model of the forth

# It's seems that the fourth model is the best combination, then let's process to change a different method.
...
summary(m5)
par(mfrow=c(1,2))
plot(m5, all.terms=TRUE, shade=TRUE, main="Model 5")
par(mfrow=c(1,1))
gam.check(m5)
...
summary(m6)
par(mfrow=c(1,2))
plot(m6, all.terms=TRUE, shade=TRUE, main="Model 6")
par(mfrow=c(1,1))
gam.check(m6)

```

The first model has a low R-squared (0.252) and the deviance explained is also very low (30%).The next models improve the performance and the best one seems to be the fifth one. We will check it better in the next steps.

```{r}
# 5. Model Comparison
# Use ANOVA to compare nested models. Note: 
# - Strictly, m4 is not nested in m3 (different structure due to by=Treatment),
#   but we can still get some insight. For nested comparisons, we focus on models
#   that differ by terms. Alternatively, compare AIC for all models.

cat("\n---------- Model Comparisons (ANOVA) ----------\n")
anova(m1, m2, m3, m4, m5, m6, test="F")
...
Comparing the anova we can observe that the model 5 is the one with the lowest residual deviance. We can observe that the change between the forth model and the fifth model is significant, so we can confirm that the change in the method was useful.
...
The adjusted R-squared indicates that 37.5% of the variability in FGm12 is explained by the model. We think we may need to include more variables to improve the model.
